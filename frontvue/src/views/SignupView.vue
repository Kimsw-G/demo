<template>
    <div>
        <div class="sign-box">
            ...
            <input type="password" placeholder="영어와 숫자를 포함한 10자리 이상 비밀번호" class="all-box " v-model="state.password"
                ref="password">
            <input type="password" placeholder="비밀번호 확인" class="all-box" v-model="state.password_chk"
                ref="password_chk">
            <input type="text" placeholder="이름" class="all-box" v-model="state.name" ref="name">
            <div>
                <input type="text" placeholder="'-' 없이 번호를 입력해주세요" class="all-box call-box" v-model="state.phone"
                    ref="phone" @keyup="isPhoneCheck" />
                <button class="all-box phone-button" ref="phoenButton" :disabled="state.isPhone"
                    @click="identify">인증하기</button>
            </div>
